...
class Storage extends EventEmitter {
	...
	constructor(type) {
		...
		try {
			...
			logger.debug(
				'Using a fake in-memory store because the host does not support storage.'
			);
	...
	encodeExpiryValue(...args) {
		return Storage.encodeExpiryValue(...args);
	}
	decodeExpiryValue(...args) {
		return Storage.decodeExpiryValue(...args);
	}
}
